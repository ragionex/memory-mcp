...
function parseArgs(argv) {
    const args = { yes: false, remove: false, scope: "auto" };
...
function detectScope() {
    ...
    const hasProject = fs.existsSync(projectMcp) &&
        fs.readFileSync(projectMcp, "utf-8").includes("ragionex-memory-mcp");
    const hasUser = fs.existsSync(userMcpJson) &&
...
async function applyOne(target, args) {
...
function detectMarkerState(content) {
...
function refreshOne(target) {
    ...
    if (state === "absent")
        return "absent";
    if (state === "modified") {
        console.error(`[ragionex-memory-mcp] Modified priority-rule markers in ${target}; auto-refresh skipped. ` +
...
export function scopeWarningOnBoot() {
    try {
        const scope = detectScope();
        if (scope === "project") {
            console.error("[ragionex-memory-mcp] Notice: project-scoped install detected (./.mcp.json). " +
...
export async function runInit(argv) {
    ...
    if (args.scope === "auto") {
        if (detected === "none") {
            console.log("Could not auto-detect a ragionex-memory-mcp install. Defaulting to USER scope (~/.claude/CLAUDE.md).");
        }
        else {
